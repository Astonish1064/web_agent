...
class LLMTaskGenerator(ITaskGenerator):
    ...
    def generate(self, seed: str, config: TaskConfig) -> List[GeneratedTask]:
        """
    ...
    def _parse_response(self, response: str, config: TaskConfig) -> List[GeneratedTask]:
        """Parse LLM response into GeneratedTask objects."""
        data = clean_json_response(response)
        if not data:
            print(f"Task Parsing Failed. Response: {response[:100]}...")
            return []
    ...
    def _sanitize_task(self, data: dict, index: int, config: TaskConfig) -> GeneratedTask:
